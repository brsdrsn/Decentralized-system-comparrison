...
# Initiates a simulator. Not really necessary for the latest version since we initiate it in the Optimizer_rnn.py
class Simulator:
    _runner: Runner
    """
    Simulator setup.
...
async def main() -> None:
    ...
    # Get the best individuals in the list
    ...
    df.to_excel('result_decentral_5.xlsx', index=False)
    best_model = model_list[best_one_index]
    torch.save(best_model, 'model_decentral_5.pth')
    ...
    for i in range(NUM_GENERATIONS):
        ...
        for i in range(len(model_list)):
            ...
            robot = ModularRobot(body, brain)
            sim = OptSimulatorCentral(model_list)
            # sim = OptSimulatorCentral()
            await sim.simulate(robot)
        best_fitness_score = max(FITNESS_LIST)
        print("best fitness of this generation:", best_fitness_score)
        list_of_models.append(model_list)
        if len(list_of_models) > 2:
            list_of_models.pop(0)
            if best_fitness_score >= best_fitness_list[-1]:
                model_list = NEXT_GENERATION[0]
                best_fitness_list.append(best_fitness_score)
            else:
                model_list = list_of_models[-2]
                print("invalid_generation, repeating last generation")
                best_fitness_list.append(  best_fitness_list[-1])
        else:
            best_fitness_list.append(best_fitness_score)
            model_list = NEXT_GENERATION[0]
        print("best fitness list: ", best_fitness_list)
        best_one_index = max(enumerate(FITNESS_LIST), key=lambda x: x[1])[0]
        best_one_index_list.append(FITNESS_LIST[best_one_index])
        best_one = model_list[best_one_index]
        best_one_list.append(best_one)
        # print("num_model_list: ", len(model_list))
        # print("num_next_generation: ", len(NEXT_GENERATION[0]))
        fitness_clear()
        #model_list = NEXT_GENERATION[0]
    ...
    df = pd.DataFrame()
    df["fitness"] = best_fitness_list
    df.to_excel('result_central.xlsx', index=False)
    best_one_off_all_index = max(enumerate(best_one_index_list), key=lambda x: x[1])[0]
    best_one_off_all = best_one_list[best_one_off_all_index]
    print("best_one_index_list:", best_one_index_list)
    print("best_one_off_all_index: ", best_one_off_all_index)
    torch.save(best_one_off_all, 'model_central_2.pth')

    brain = CBrain(best_one_off_all)
    robot = ModularRobot(body, brain)
    sim = Simulator()
    await sim.simulate(robot)
    """
